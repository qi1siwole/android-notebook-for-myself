...
public class ConditionStateGroup extends ConditionState {
    ...
    public String getOperatorString() {
        return getOperatorString(mOperatorType);
    }
    ...
    public void resetOperatorType(int type) {
        _setOperatorType(type);
        resetOperands();
    }

    public boolean modifyOperatorType(int type) {
        if (TYPE_DIFFERENCE_SET == type && mOperands.size() > MAX_OPERAND_COUNT) {
            return false;
        }
        _setOperatorType(type);
        return true;
    ...
    private void _setOperatorType(int type) {
        mOperatorType = type;
    ...
    private static List<Integer> sOperatorTypeList;

    public static List<Integer> getOperatorTypeList() {
        if (null == sOperatorTypeList) {
            sOperatorTypeList = new ArrayList<>();
            sOperatorTypeList.add(TYPE_INTERSECTION);
            sOperatorTypeList.add(TYPE_UNION);
            sOperatorTypeList.add(TYPE_DIFFERENCE_SET);
        }
        return sOperatorTypeList;
    }
    ...
    public boolean canRemoveOperand() {
        return mOperands.size() > MIN_OPERAND_COUNT;
    }
    ...
    public int addOperand(Condition condition) {
        ...
        condition.setParentCondition(getCondition());

        return mOperands.size() - 1;
    }

    public void removeOperand(Condition operand) {
        if (!canRemoveOperand()) {
            return;
        }
        mOperands.remove(operand);
    }
